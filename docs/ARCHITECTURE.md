...
Il registro delle fonti in `src/lib/sources.ts` è affiancato da `scripts/audit/controlla_fonti.py`. Il controllo mensile del 15, eseguito da `.github/workflows/source-audit.yml`, esclude intenzionalmente il Radar Google News, verifica le fonti ufficiali e confronta una fingerprint degli endpoint dati con la baseline persistita nella cache GitHub Actions. Ogni run salva inoltre un report JSON come artifact GitHub. Il controllo è non distruttivo e non aggiorna i dataset pubblicati.
