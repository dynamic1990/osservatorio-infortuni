...
export function MalattiePatologieWidget() {
  ...
  const casiND = dati.categorie.find((c) => c.key === "ND")?.casi ?? 0;
  const casiAltro = dati.categorie.find((c) => c.key === "ALTRO")?.casi ?? 0;

  const maxCasi = Math.max(...categorie.map((c) => c.casi), 1);
  ...
  const righeDettaglio = (c: (typeof categorie)[number]): RigaDettaglio[] => {
    ...
    righe.push({ label: "Denunce totali", valore: exactNumber(c.casi) });
  ...
  return (
    <div style={{ display: "grid", gap: "var(--space-4)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", flexWrap: "wrap", gap: "var(--space-2)", fontSize: "0.88rem", fontWeight: 700 }}>
        <span>Patologie denunciate per gruppo clinico (ICD-10)</span>
        <span style={{ fontSize: "0.78rem", color: "var(--color-text-soft)", fontWeight: 400 }}>
          Tocca una patologia per i codici ICD-10 più frequenti
        </span>
      </div>
      ...
      <div style={{ display: "grid", gap: "var(--space-2)" }}>
        {categorie.map((c, idx) => {
          ...
          const larghezza = Math.max(6, Math.round((c.casi / maxCasi) * 100));
          return (
            <div
              key={c.key}
              style={{
                ...
                <span style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <span
                    style={{
                      ...
                      borderRadius: 2,
                      marginTop: 5,
                      background: `linear-gradient(to right, ${colorFor(idx)} ${larghezza}%, rgba(0,0,0,0.07) ${larghezza}%)`,
                    }}
                  />
                </span>
      ...
      <p className="source-note">
        Raggruppamento clinico dei codici ICD-10 presenti nel dataset INAIL delle denunce protocollate.
        Le patologie muscolo-scheletriche e del tessuto connettivo (capitolo M) dominano il quadro, con
        rachide e spalla in testa; i tumori professionali e le patologie da polveri emergono per gravità
        anche se numericamente minori. Il codice non indicato (ND) e le patologie residue (ALTRO) sono
        esclusi dal ranking: rappresentano rispettivamente {exactNumber(casiND)} e {exactNumber(casiAltro)} denunce.
      </p>
    </div>
  );
}
